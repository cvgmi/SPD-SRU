...
from matrixcell import NormalCNNCell

from readdata import read_data


def get_a_cell():
    return tf.contrib.rnn.BasicLSTMCell(num_units = params_spatial_dim * out_channel)
...
CNN_num_layer = 1
CNN_kernel_shape = [[7,7,10],[7,7,out_channel]]


params_spatial_dim = 30 #4800
beta = 0.3
...
CNNRNNcell = [NormalCNNCell(num_layer = CNN_num_layer, kernel_shape = CNN_kernel_shape, batch_size = batch_size , in_channel= in_channel, out_channel=out_channel , keep_prob = keep_prob  , output_dims = 7500 )]
...
output_series = tf.reshape(outputs,[batch_size,-1])
# pdb.set_trace()
